# Homesteaders-u4m - v1.6.4

# Homesteaders-u4m for the HubSpot CMS by New Breed
...
```
hs upload src homesteaders-u4m
```

#### Watch entire project

```
hs watch src homesteaders-u4m
```

#### Fetch entire project with overwriting

```
hs fetch homesteaders-u4m src -o
```

#### Fetch a specific module add -o to the end to overwrite files

```
hs fetch homesteaders-u4m/modules/u4m-000.module src/modules/u4m-000.module -o
```

hs fetch homesteaders-u4m/modules/u4m-cards-side.module src/modules/u4m-cards-side.module -o

#### Watch entire folder on non-default portal (portal names are set in hubspot.config.yml)

```
hs watch --account=hug_vt src homesteaders-u4m
```
